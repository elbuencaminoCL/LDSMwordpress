...
function wpse_setup_theme() {
    ...
    if ( function_exists( 'add_image_size' ) ) { 
        ...
        add_image_size( 'lang', 2000, 280, array( 'center', 'center'));
        add_image_size( 'generica', 540, 360, array( 'center', 'center'));
        add_image_size( 'news', 300, 200, array( 'center', 'center'));
        add_image_size( 'news-home', 280, 160, array( 'center', 'center'));
        ...
        add_image_size( 'act', 378, 240, array( 'center', 'center'));
    }
} 
...
function my_custom_login_logo() {
    echo '<style type="text/css">
        ...
        h1 a { background-image:url('.get_bloginfo('template_directory').'/imag/logo/logo_ldsm_admin.png) !important; background-size:320px 67px !important; width:320px !important; height:67px !important;}
    </style>';
}
        ...
        foreach($home_pages as $hpages){
            ...
            if($hpages->menu_order >= 0){
                echo '<div class="'.$pos.' col-md-4 col-xs-12">';
                    echo '<div class="row">';
                        echo '<div class="col-xs-12 ciclo escondido">';
                            echo '<div class="esfera">';
                                echo get_the_post_thumbnail($hpages->ID, 'child', array('class' => 'img-responsive'));
                            echo '</div>';
                        echo '</div>';
                        echo '<div class="col-xs-12">';
                            echo '<h4><a href="'.get_the_permalink($hpages).'">'.$hpages->post_title.'</a></h4>';
    ...
    register_post_type( 'actividades',
        array(
            'labels' => array(
                ...
                'add_new' => __('Agregar Academia'),
                'add_new_item' => __('Agregar nueva Academia'),
                'edit_item' => __('Editar Academia'),
                'new_item' => __('Nueva Academia'),
                'all_items' => __('Todas las Academias'),
                'view_item' => __('Ver Academias'),
                'search_items' => __('Buscar Academias')
            ),
            'public' => true,
            'has_archive' => true,
            'rewrite' => array('slug' => 'ver-academias', 'hierarchical' => true),
            'hierarchical' => true,
            'show_ui' => true,
            'query_var' => true,
...
//=================================================================== WORDPRESS WIDGETS// 
    function ldsm_widgets_init() {
		register_sidebar(array(
            'name' => __('Sidebar General', 'ldsm'),
            'description' => __('Sidebar general sitio web', 'ldsm'),
            'id' => 'sidebar-general',
            'before_title' => '<h2>',
            'after_title' => '</h2>',
            'before_widget' => '',
    ...
    add_action('widgets_init', 'ldsm_widgets_init');
//
?>